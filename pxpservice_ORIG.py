#!/usr/bin/python
#import sys;sys.path.append(r'/Applications/Eclipse.app/Contents/Eclipse/plugins/org.python.pydev_5.4.0.201611281236/pysrc')
#import pydevd;
...
class encTeradek(encDevice):
    ...
    def discover(self, callback):
        ...
        if(enc.code & enc.STAT_LIVE):
            dbg.prn(dbg.MN, "encTDK.discovery is disabled due to live mode...")
            return
        def discovered(results):
            global sockCmd
            global devaddingnow
            ...
            if (pu.mdbg.checkscf(sockCmd, pu.SCF_SHOWBONJ)):
                dbg.prn(dbg.TDK, "--> TDK camera found.....")
            ...
            recs = pu.bonjour.parseRecord(results['txtRecord'])
            if (pu.mdbg.checkscf(sockCmd, pu.SCF_SHOWBONJ)):
                dbg.prn(dbg.TDK,"-->Teradek:recs-->{}".format(results))
            output = {}
    ...
    def discover(self, callback):
        ...
        try:
            if(enc.code & enc.STAT_SHUTDOWN):
                return
            if(enc.code & enc.STAT_LIVE):
                dbg.prn(dbg.MN, "encMatrox.discovery is disabled due to live mode...")
                return
...
class encPivothead(encDevice):
    ...
    def discover(self, callback):
        ...
        def discovered(results):
            global devaddingnow
            if(enc.code & enc.STAT_LIVE):
                dbg.prn(dbg.MN, "encPivothead.discovery is disabled due to live mode...")
                return
            recs = pu.bonjour.parseRecord(results['txtRecord'])
            output = {}
            # url to the stream, consists of:
...
class encSonySNC(encDevice):
    ...
    def discover(self, callback): # 'self' is not defined until it hits __init__, do not use self.XXX class attributes
        ...
        try:
            ...
            if(enc.code & enc.STAT_LIVE):
                dbg.prn(dbg.MN, "encSonySNC.discovery is disabled due to live mode...")
                return
            #to find ALL ssdp devices simply enter ssdp:all as the target
    ...
    def monitor(self):
        ...
        try:
            ...
            if ((enc.code & enc.STAT_LIVE) and not self.evt_stat and not self.rec_stat_worker):
                ...
                self.rec_stat_worker.start()
                dbg.prn(dbg.SRM, "rec_stat just posted: done:{}".format(str(self.rec_stat_worker.done)))
            else:
...
if __name__=='__main__': # will enter here only when executing this file (if it is included from another process, this IF condition will be false)
    try:
        ...
        try:
            ...
            pass
        try:
            os.system("rm /tmp/pxp-url* 2>/dev/null")
        except:
            pass 
